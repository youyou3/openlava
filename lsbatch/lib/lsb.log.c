...
struct eventRec * lsbGetNextJobEvent(struct eventLogHandle *,
                                     ...
                                     LS_LONG_INT *,
                                     struct jobIdIndexS *);
static struct eventRec * lsbGetNextJobRecFromFile(FILE *,
                                                  int *,
                                                  int,
                                                  LS_LONG_INT *);
...
lsb_openelog(struct eventLogFile *ePtr, int *lineNum)
{
    ...
    if (curOpenFile >= 0) {
        ...
        if (curOpenFile == 0)  {
            ...
            if (fscanf(elog_fp, "%c%d ", &ch, &pos) != 2 || ch != '#') {
                ls_syslog(LOG_WARNING, I18N(5501,
                                            "%s: fscanf(%s) failed: event file is old format"), __func__, eventFile);
                pos = 0;
            } else {
                *lineNum = 1;
        ...
        ls_syslog(LOG_ERR, _i18n_msg_get(ls_catd , NL_SETN, 5505,
                                         "%s: current open event file number < 0"), __func__); /* catgets 5505 */
        return  NULL;
    }
...
lsb_geteventrecord(FILE *log_fp, int *LineNum)
{
    ...
    /* read the record and build the logRec data
     * structure.
     */
    readEventRecord(line, logRec);
 ...
 */
static int
readNewJgrp(char *line, struct jgrpLog *jgrp)
{
    int cc;
    int n;
 ...
 */
static int
writeNewJgrp(FILE *fp, struct jgrpLog *jgrp)
{
    if (addQStr(fp, jgrp->name) < 0)
        return LSBE_SYS_CALL;

    if (addQStr(fp, jgrp->path) < 0)
        return LSBE_SYS_CALL;

    if (addQStr(fp, jgrp->user) < 0)
        return LSBE_SYS_CALL;
 ...
 */
static int
writeDelJgrp(FILE *fp, struct jgrpLog *jgrp)
{
    if (addQStr(fp, jgrp->path) < 0)
        return LSBE_SYS_CALL;
